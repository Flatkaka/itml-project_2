...
class FlappyAgent:
    def __init__(self, a=0.1,g=1):
        self.results = []

        self.discountFactor = 0.1
        self.QL = QL(alpha=a,gamma = g)
        self.actions = [0,1]
        self.curr_epi = dict()
        self.score = 0
...
gamma = [1,0.975,0.95,0.925,0.9]
alphas = [0.1,0.15,0.175,0.2,0.225]
results = [[],[],[],[]]
max_avrage_list = [[],[],[],[]]
...
for g in gamma:
    for a in alphas:
        agent = FlappyAgent(a,g)
        results = train(3000000, agent,a,g,results)
        avrage = run_game(250, agent)
        max_avrage_list[0].append(avrage)
        max_avrage_list[1].append(a)
        max_avrage_list[2].append(g)
        max_avrage_list[3].append(agent)
...
print("that alpha was: %f" % max_alpha)
print("that gamma was: %f" % max_gamma)
pickle.dump(agent, open('task3QL.txt',"wb"))



data = {"Count":results[1], "Avrage":results[0], "Alpha":results[2],"Gamma":results[3]}
df = pd.DataFrame(data)

# sns.relplot(x="Count", y="Avrage",hue='Alpha', kind="line", data=df,size="Gamma")


sns.relplot(x="Count", y="Avrage", hue="Alpha",
